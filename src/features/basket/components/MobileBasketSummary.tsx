    ...
    <div className="flex flex-col flex-1 bg-cream">
      <div className="px-4 py-5 flex flex-col gap-5">
        ...
        <section>
          ...
          <ul className="border border-sand rounded-xl divide-y divide-sand bg-white overflow-hidden">
            {selectedAddresses.map((addr) => (
              <li
                key={addr.id}
                className="flex items-center justify-between px-3 py-2.5"
              >
                <span className="text-sm text-navy truncate pr-3">
                  {addr.displayAddress}
                </span>
                <button
        ...
        {letterSnippet && (
          <section>
            <h2 className="text-sm font-semibold text-navy mb-2">Your letter</h2>
            <div className="border border-sand rounded-xl bg-white px-4 py-3">
        ...
        {/* Pricing */}
        <section>
          <h2 className="text-sm font-semibold text-navy mb-2">Pricing</h2>
          <div className="border border-sand rounded-xl bg-white divide-y divide-sand overflow-hidden">
            <PricingRow
              label="Pay as you go"
              subLabel="per letter"
        ...
        {/* Order total */}
        <section className="bg-white border border-sand rounded-xl px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-navy">Order total</div>
      ...
      <StickyCTA
        hint={
          <span className="text-[11px] text-navy-soft/70">
...
function PricingRow({
  ...
  return (
    <div
      className={`flex items-center justify-between px-4 py-3 ${highlight ? "bg-cream" : ""}`}
    >
      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-navy">{label}</span>
          {badge && (
            <span className="text-[10px] bg-coral-soft text-coral-dark px-1.5 py-0.5 rounded font-medium">
              {badge}
            </span>
          )}
        </div>
        {subLabel && <span className="text-xs text-navy-soft/70">{subLabel}</span>}
      </div>
      <div className="text-right">
        <div className="text-sm font-semibold text-navy">{formatPence(price)}</div>
        {saving !== undefined && saving > 0 && (
          <div className="text-xs text-navy-soft/70">Save {formatPence(saving)}</div>
        )}
      </div>
    </div>
  )
