  ...
  if (selectedAddresses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center flex-1 gap-4 text-center px-4">
        <p className="text-navy-soft">Your basket is empty.</p>
        <Link href="/map" className="text-coral font-medium hover:underline">
  ...
  return (
    <div className="max-w-3xl mx-auto w-full px-4 py-8 flex flex-col gap-8">
      ...
      <section>
        ...
        <ul className="border border-sand rounded-xl divide-y divide-sand overflow-hidden">
          {selectedAddresses.map((addr) => (
            <li key={addr.id} className="flex items-center justify-between px-4 py-3 bg-white">
              <span className="text-sm text-navy">{addr.displayAddress}</span>
              <button
                onClick={() => handleRemoveAddress(addr.id)}
                className="text-xs text-navy-soft/70 hover:text-red-500 transition-colors ml-4 shrink-0"
        ...
        <Link href="/map" className="text-xs text-coral hover:underline mt-2 inline-block">
          ← Edit addresses
        </Link>
      </section>
      ...
      {letterSnippet && (
        <section>
          <h2 className="text-lg font-semibold text-navy mb-3">Your letter</h2>
          <div className="border border-sand rounded-xl bg-white px-5 py-4">
            <p className="text-sm text-navy-soft whitespace-pre-line font-serif leading-relaxed">
              {letterSnippet}
            </p>
          </div>
          <Link href="/letter" className="text-xs text-coral hover:underline mt-2 inline-block">
            ← Edit letter
          </Link>
        </section>
      )}
      ...
      {/* Pricing */}
      <section>
        <h2 className="text-lg font-semibold text-navy mb-3">Pricing</h2>
        <PricingTable letterCount={selectedAddresses.length} />
      </section>

      {/* Order total + CTA */}
      <section className="bg-white border border-sand rounded-xl px-5 py-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-base font-semibold text-navy">Order total</div>
            <div className="text-sm text-navy-soft">
              {selectedAddresses.length} letter{selectedAddresses.length !== 1 ? "s" : ""} · inc.
              postage &amp; printing
            </div>
          </div>
          <div className="text-2xl font-bold text-coral">{formatPence(totalPence)}</div>
        </div>
        ...
        <button
          ...
          className="w-full py-3 rounded-lg font-semibold text-base transition-colors disabled:opacity-40 disabled:cursor-not-allowed bg-coral hover:bg-coral-dark text-white"
        >
          Send {selectedAddresses.length} {selectedAddresses.length === 1 ? "letter" : "letters"} —{" "}
          {formatPence(totalPence)}
        </button>
        <p className="text-xs text-navy-soft/70 text-center">
